...
from models.samp_net import _adaptive_avg_pool2d, _manual_adaptive_avg_pool2d  # noqa: E402
...
@pytest.mark.parametrize("output_size", [(2, 2), (3, 3), (4, 4), (8, 8)])
def test_manual_pool_matches_torch_for_non_divisible_shapes(output_size):
    tensor = torch.randn(2, 3, 7, 7)
    expected = F.adaptive_avg_pool2d(tensor, output_size)
    actual = _manual_adaptive_avg_pool2d(tensor, output_size)
    torch.testing.assert_close(actual, expected)


def test_manual_pool_preserves_autograd():
    tensor = torch.randn(1, 2, 7, 7, requires_grad=True)
    _manual_adaptive_avg_pool2d(tensor, (3, 3)).sum().backward()
