...
class TestPersonsDateFiltering:
    def _make_person_db(self, tmp_path, photos):
        db_path = str(tmp_path / "test.db")
        _make_db(
            db_path,
            photos=photos,
            persons=[(1, "Alice", 3)],
            faces=[(i + 1, p["path"], 1) for i, p in enumerate(photos)],
        )
        return db_path

    def _patches(self, db_path):
        return (
            mock.patch("api.routers.persons.get_db_connection", _conn_factory(db_path)),
            mock.patch("api.routers.persons.VIEWER_CONFIG", _VIEWER_CONFIG),
            mock.patch("api.db_helpers._existing_columns_cache", None),
        )

    def test_date_range(self, tmp_path):
        """Filters person photos to date range via to_exif_date."""
        photos = [
            _photo("/p1.jpg", "2024:06:15 10:00:00", category="portrait"),
            _photo("/p2.jpg", "2024:09:20 14:00:00", category="portrait"),
        ]
        db_path = self._make_person_db(tmp_path, photos)
        app = _create_app_no_auth()
        p = self._patches(db_path)
        with p[0], p[1], p[2]:
            resp = TestClient(app).get(
                "/api/persons/1/photos?page=1&date_from=2024-06-01&date_to=2024-06-30"
            )
        assert resp.status_code == 200
        result = resp.json()["photos"]
        assert len(result) == 1
        assert result[0]["path"] == "/p1.jpg"

    def test_date_from_only(self, tmp_path):
        photos = [
            _photo("/old.jpg", "2023:01:01 08:00:00", category="portrait"),
            _photo("/new.jpg", "2024:06:15 12:00:00", category="portrait"),
        ]
        db_path = self._make_person_db(tmp_path, photos)
        app = _create_app_no_auth()
        p = self._patches(db_path)
        with p[0], p[1], p[2]:
            resp = TestClient(app).get(
                "/api/persons/1/photos?page=1&date_from=2024-01-01"
            )
        assert len(resp.json()["photos"]) == 1
        assert resp.json()["photos"][0]["path"] == "/new.jpg"
    ...
    def test_date_to_only(self, tmp_path):
        ...
        with p[0], p[1], p[2]:
            resp = TestClient(app).get(
                "/api/persons/1/photos?page=1&date_to=2023-12-31"
            )
        assert len(resp.json()["photos"]) == 1
        assert resp.json()["photos"][0]["path"] == "/old.jpg"

    def test_no_dates_returns_all(self, tmp_path):
        photos = [
            _photo("/p1.jpg", "2024:06:15 10:00:00", category="portrait"),
            _photo("/p2.jpg", "2024:09:20 14:00:00", category="portrait"),
        ]
        ...
        p = self._patches(db_path)
        with p[0], p[1], p[2]:
            resp = TestClient(app).get("/api/persons/1/photos?page=1")
        assert len(resp.json()["photos"]) == 2

    def test_date_to_includes_end_of_day(self, tmp_path):
        """to_exif_date + '23:59:59' suffix includes late evening photos."""
        photos = [
            _photo("/late.jpg", "2024:06:30 23:45:00", category="portrait"),
        ]
        db_path = self._make_person_db(tmp_path, photos)
        app = _create_app_no_auth()
        p = self._patches(db_path)
        with p[0], p[1], p[2]:
            resp = TestClient(app).get(
                "/api/persons/1/photos?page=1&date_to=2024-06-30"
            )
        assert len(resp.json()["photos"]) == 1
