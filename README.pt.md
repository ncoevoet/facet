...
1. **Escanear** — Aponte o Facet para uma pasta de fotos. Cada imagem é analisada quanto à qualidade, composição e rostos. Suporta JPG, HEIF/HEIC/HIF (as fotos `.HIF` Canon HDR PQ são decodificadas com a função de transferência SMPTE ST 2084 PQ e automaticamente convertidas para sRGB SDR), 10 formatos RAW (CR2, CR3, NEF, ARW, RAF, RW2, DNG, ORF, SRW, PEF) e imagens PNG, GIF, WebP, BMP, TIFF e AVIF (o AVIF precisa de um Pillow compilado com suporte a AVIF, nativo a partir de 11.3). Um RAW que o LibRaw não consegue decodificar — como um DNG do Lightroom "Mesclar para HDR/Panorama" (`-HDR.dng`, `-Pano.dng`) ou uma câmera mais recente do que a versão do LibRaw instalada — recorre à sua maior pré-visualização incorporada via `exiftool` (precisa de uma pré-visualização de pelo menos 1024 px no lado maior; as pontuações vêm então dessa pré-visualização, não de uma decodificação RAW completa). As fusões do Lightroom Classic 13+ (e Enhance) armazenam essa pré-visualização como JPEG XL; o Facet a decodifica diretamente.
