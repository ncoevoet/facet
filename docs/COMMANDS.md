...
>
> **Caveats.** The photo-side timestamp for *newest-wins* is `scanned_at` (the last scan), not a per-rating edit time — so a sidecar newer than the last scan can override a rating you changed in Facet *after* that scan. Run `--import-sidecars` before re-rating in Facet if the external editor is the source of truth. The CLI `--import-sidecars` / `--export-sidecars` operate on the **global single-user** rating columns; in multi-user mode per-user ratings live in `user_preferences` and are not surfaced by these CLI commands. If you use the `photo_tags` lookup table, run `python database.py --migrate-tags` after importing.
