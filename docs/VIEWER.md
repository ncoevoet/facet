...
| `PUT /api/albums/{id}/scoring_context` | `[Edition]` Set the album's scoring context; materializes it onto members matching the album right now (smart albums resolve `smart_filter_json` live), returns `{updated, conflicts}` |
