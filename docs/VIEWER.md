...
| `PUT /api/albums/{id}/scoring_context` | `[Edition]` Set the album's scoring context; materializes it onto member photos, returns `{updated, conflicts}` |
...
| `POST /api/scan/recompute` | `[Edition]` Trigger a full-library aggregate recompute (`--recompute-average`) as a background job; shares the scan job lock, so it cannot run alongside a directory scan. Unlike `/start`, its argv is fixed server-side and takes no request input, so it needs no superadmin |
