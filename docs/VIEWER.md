...
| `POST /api/cull/apply` | `[Edition]` Copy keeps or move/trash rejects to a folder (see [Cull to folder](#cull-to-folder)). Body `{paths?, filters?, action, target_dir?, include_companions, include_sequence_siblings, dry_run}` — either `paths` or a gallery `filters` set; `action` is `copy_keeps \| trash_rejects \| move_rejects`; `include_companions` (default `false`) adds each file's same-stem companion RAW and `.xmp`; `include_sequence_siblings` (default `false`) adds every other frame sharing a matched photo's `(sequence_kind, sequence_group_id)`; `dry_run` defaults to `true`. Response adds `matched` (paths that qualified for the action) and `sequence_siblings` (sibling frame count, reported even when the flag is off) alongside `skipped` / `excluded_by_state` / `not_visible` |
