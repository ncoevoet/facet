...
| `GET /api/photos/paths` | `{ total, paths }` — tous les chemins correspondants, sans tri ; utilisé à la demande par les actions qui ont besoin des noms de fichiers (téléchargement, copie), jamais par « Tout sélectionner ». Plafonné à 10000 : une vue qui en contient davantage est refusée avec un `412` indiquant le nombre et le plafond plutôt que tronquée — une liste de chemins partielle serait une sélection que l'utilisateur croit complète — de sorte qu'une vue plus large se rabat sur le `GET /api/photos/count`, qui ne renvoie qu'un décompte |
...
| `POST /api/export/sidecars` | `[Edition]` Écrire des sidecars pour des chemins explicites ou un ensemble de filtres. Corps `{paths?, filters?, exclude?, overwrite}` — exactement l'un de `paths` (max 10000) ou un ensemble `filters` de galerie : nommer les deux renvoie un `422`, n'en nommer aucun un `400`. `exclude` (optionnel, max 1000) retire des chemins nommés d'un ensemble `filters` ; `filters` est résolu via la même normalisation que celle utilisée par `GET /api/photos`, exactement comme `POST /api/cull/apply` ci-dessous, et un ensemble `filters` nommant un album est soumis au même contrôle d'accès que le GET de cet album lui-même — `404` pour un album qui n'existe pas, `403` pour celui d'un autre utilisateur sur une installation à accès contrôlé. Un ensemble `filters` correspondant à plus de 10000 photos est refusé avec un `412` indiquant le nombre et le plafond, compté avant que toute liste de chemins soit construite, et jamais tronqué |
