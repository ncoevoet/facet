...
Facet étiquette chaque photo avec le « moment » de l'événement qu'elle dépeint — pour un mariage : préparatifs (mariée/marié), entrée de cérémonie, vœux, échange des alliances, premier baiser, sortie de cérémonie, photos de famille, portraits du couple, cocktail, détails de la réception, dîner/discours, découpe du gâteau, première danse, soirée, départ — ou `other`. Ni Narrative Select ni AfterShoot ne le font ; ils regroupent uniquement par temps et similarité visuelle.

C'est **zero-shot et entièrement local** : l'embedding CLIP/SigLIP stocké est comparé par similarité cosinus à des prompts textuels moyennés pour chaque moment (L0), ajusté par de petits a priori de visage/tag (L1), puis **lissé le long de la chronologie** par une passe de Viterbi de sorte qu'une lecture isolée erronée soit ramenée dans la suite environnante (L2). Un départage VLM facultatif (L3, 16gb/24gb) peut réévaluer les images à faible confiance. Parce qu'il ne s'agit que d'un produit scalaire sur des embeddings déjà présents dans la base de données — pas de décodage d'image, pas de passe de modèle par image — c'est peu coûteux et **s'exécute automatiquement à la fin de chaque scan** ; relancez toute la bibliothèque avec `python facet.py --recompute-moments`.

Les moments apparaissent comme titres de scène et comme filtre de galerie (`GET /api/photos?narrative_moment=vows`, options depuis `GET /api/filter_options/narrative_moments`). Le vocabulaire est piloté par la configuration selon le type d'événement — voir [Configuration — Narrative Moments](CONFIGURATION.md#narrative-moments) pour ajuster les prompts/seuils ou ajouter un genre autre que le mariage.
