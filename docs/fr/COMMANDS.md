...
| `python facet.py --detect-sequences` | Détecte les séries multi-images délibérées : d'abord les bracketings d'exposition d'après les EXIF stockés, puis les panoramas d'après la géométrie des vignettes, et place la vignette principale de chaque rafale sur l'exposition de référence. Exécuté à la fin de chaque scan et comme étape de `--upgrade-db` |
