...
| `python facet.py --detect-sequences` | Détecte les séries de bracketing d'exposition à partir des EXIF stockés (bibliothèque entière, aucun décodage d'image) et place la vignette principale de chaque rafale sur l'exposition de référence. Exécuté à la fin de chaque scan et comme étape de `--upgrade-db` |
| `python facet.py --detect-panoramas` | Détecter les panoramas en appariant géométriquement les vignettes stockées (bibliothèque entière, CPU, aucun décodage d'image). S'exécute à la fin de chaque analyse |
