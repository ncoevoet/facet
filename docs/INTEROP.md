...
**Manifest version 2.** The manifest now also carries `burst_group_id`, `sequence_kind`, `sequence_group_id` and `score_stars`, which the plug-in uses for the burst pick/reject and star-fallback options below. There is no backward-compatible read path: a plug-in built for version 2 refuses a version-1 manifest outright with a dialog telling you to re-export, and an older plug-in cannot read a version-2 manifest either. If you see that dialog, just re-run `--export-manifest`.
