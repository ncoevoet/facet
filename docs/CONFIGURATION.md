...
A scoring context is assigned **per album** via `PUT /api/albums/{id}/scoring_context` (edition-gated), which writes the album's `scoring_context` column and materializes the same value into `photo_scoring_overrides.scoring_context` for every photo that is a member **at that moment** — the response's `conflicts` count is how many of those already carried a different context (last-write-wins), and `updated` is how many were actually written. A manual album resolves membership from its `album_photos` rows; a smart album (`is_smart`) has none, so membership is instead resolved by evaluating its saved `smart_filter_json` against the live database, the same way the album's own gallery view resolves it (`_resolve_album_member_paths`, mirroring the smart branch of `_fetch_album_photos`) — a smart album whose filter currently matches nothing comes back with `updated: 0` and a `warning`. Either way the stamp is a snapshot, not a subscription: for a manual album, a photo added *afterward* via `append_album_photos` does inherit the context automatically, but a smart album's filter matching a *new* photo later does **not** retroactively apply the context to it — only calling `PUT` again re-stamps whatever currently matches. `DELETE /api/albums/{id}/scoring_context` clears the album's `scoring_context` and undoes the stamp on exactly the members this album set (`source = 'album:<id>'`), leaving any photo's own manual override, or a context a different album set, untouched. `GET /api/albums/{id}/suggested_context` proposes a context from the album's dominant `narrative_moment`, mapped through `suggest_from_moments`, along with the dominant moment's `share` of the album — it writes nothing; the assignment above still has to be called explicitly.
