...
**Ordine effettivo** = `promote` (in ordine) → l'ordine di priorità globale meno i nomi promossi ed esclusi → `default` (la categoria di ripiego, priorità 999) per ultima. `default` stessa non può mai essere promossa o esclusa. Non passare alcun contesto, o passare un nome di contesto sconosciuto, ricade sul semplice ordine di priorità globale (registrando un avviso nel log per un nome sconosciuto); l'elenco risolto `[(category_name, CategoryFilter)]` viene memorizzato in cache (memoize) per nome di contesto per l'intera durata della configurazione caricata (`ScoringConfig.resolve_context_order`).
...
Un contesto di punteggio si assegna **per album** tramite `PUT /api/albums/{id}/scoring_context` (riservato alla modalità di modifica), che scrive la colonna `scoring_context` dell'album e materializza lo stesso valore in `photo_scoring_overrides.scoring_context` per ogni membro attuale — il conteggio `conflicts` nella risposta indica quanti membri portavano già un contesto diverso (vince l'ultima scrittura; sta al chiamante decidere se avvisare prima di confermare). Le foto aggiunte all'album in seguito ereditano automaticamente il suo contesto (`append_album_photos`). `GET /api/albums/{id}/suggested_context` propone un contesto a partire dal `narrative_moment` dominante dell'album, mappato tramite `suggest_from_moments`, insieme alla `share` (quota) del momento dominante nell'album — non scrive nulla; l'assegnazione sopra descritta deve comunque essere richiamata esplicitamente.
...
**Cambiare le priorità richiede un ricalcolo.** Il riordino in sé non tocca la `category` memorizzata di alcuna foto — esegui `python facet.py --recompute-average`, oppure attiva `POST /api/scan/recompute` dal viewer (riservato alla modalità di modifica; condivide il lock del job di scansione, quindi non può essere eseguito insieme a una scansione di directory) e interroga `GET /api/scan/recompute_status`. `update_all_aggregates` emette righe `@FACET_PROGRESS` (in precedenza solo `tqdm`), così da poter pilotare una barra di avanzamento.
