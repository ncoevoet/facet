...
Preset forniti di serie — tutto semplice JSON modificabile dall'utente; `default` è un delta vuoto, quindi il comportamento esistente resta invariato finché non viene assegnato esplicitamente un contesto:
...
Un contesto di punteggio si assegna **per album** tramite `PUT /api/albums/{id}/scoring_context` (riservato alla modalità di modifica), che scrive la colonna `scoring_context` dell'album e materializza lo stesso valore in `photo_scoring_overrides.scoring_context` per ogni foto che è membro **in quel momento** — il conteggio `conflicts` nella risposta indica quante di esse portavano già un contesto diverso (vince l'ultima scrittura), e `updated` indica quante sono state effettivamente scritte. Un album manuale risolve l'appartenenza dalle sue righe `album_photos`; un album intelligente (`is_smart`) non ne ha, quindi l'appartenenza viene invece risolta valutando il suo `smart_filter_json` salvato rispetto al database live, allo stesso modo in cui la vista galleria dell'album stesso la risolve (`_resolve_album_member_paths`, che rispecchia il ramo "intelligente" di `_fetch_album_photos`) — un album intelligente il cui filtro al momento non corrisponde a nulla restituisce `updated: 0` con un `warning`. In entrambi i casi la marcatura è un'istantanea, non un abbonamento: per un album manuale, una foto aggiunta *in seguito* eredita comunque automaticamente il contesto, ma una foto che corrisponde *più tardi* al filtro di un album intelligente **non** eredita retroattivamente il contesto — solo un nuovo `PUT` ri-marca le corrispondenze attuali. `DELETE /api/albums/{id}/scoring_context` cancella lo `scoring_context` dell'album e annulla la marcatura esattamente sui membri che questo album aveva impostato (`source = 'album:<id>'`), senza toccare la sovrascrittura manuale di una foto né un contesto impostato da un altro album. `GET /api/albums/{id}/suggested_context` propone un contesto a partire dal `narrative_moment` dominante dell'album, mappato tramite `suggest_from_moments`, insieme alla `share` (quota) del momento dominante nell'album — non scrive nulla; l'assegnazione sopra descritta deve comunque essere richiamata esplicitamente.
