...
| `python facet.py --detect-sequences` | Rileva le serie di bracketing di esposizione dagli EXIF memorizzati (intera libreria, nessuna decodifica delle immagini) e sposta la foto guida di ogni raffica sull'esposizione di base. Viene eseguito al termine di ogni scansione e come passo di `--upgrade-db` |
| `python facet.py --detect-panoramas` | Rilevare i panorami confrontando geometricamente le miniature memorizzate (intera libreria, CPU, nessuna decodifica). Viene eseguito al termine di ogni scansione |
