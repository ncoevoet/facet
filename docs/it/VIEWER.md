...
| `POST /api/cull/apply` | `[Edition]` Copia i mantenuti o sposta/cestina gli scartati in una cartella (vedi [Scarta in cartella](#scarta-in-cartella)). Corpo `{paths?, filters?, action, target_dir?, include_companions, include_sequence_siblings, dry_run}` — o `paths` oppure un insieme `filters` di galleria; `action` è `copy_keeps \| trash_rejects \| move_rejects`; `include_companions` (predefinito `false`) aggiunge il RAW e l'`.xmp` di corredo con lo stesso nome base di ogni file; `include_sequence_siblings` (predefinito `false`) aggiunge ogni altro fotogramma che condivide `(sequence_kind, sequence_group_id)` con una foto corrispondente; `dry_run` predefinito a `true`. La risposta aggiunge `matched` (percorsi idonei per l'azione) e `sequence_siblings` (conteggio dei fotogrammi della serie, segnalato anche a interruttore disattivato) accanto a `skipped` / `excluded_by_state` / `not_visible` |
