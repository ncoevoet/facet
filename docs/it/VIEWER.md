...
- **Contesti di punteggio** — `GET /api/config/scoring_contexts` elenca i preset configurati (`default`, `action_stage`, `party_event`, `portrait_session`, `wildlife`, `landscape`, `motorsport`) insieme all'ordine effettivo risolto di ciascuno. Un contesto promuove alcune categorie in testa ed esclude del tutto le altre senza toccare l'ordine globale. Il delta `promote`/`excluded` di ogni preset non-`default` è a sua volta modificabile dalla stessa scheda **Contesto di punteggio** — trascina la testa promossa nell'ordine voluto, attiva/disattiva l'esclusione di una categoria, salva — tramite `PUT /api/config/scoring_contexts/{name}` (riservato alla modalità edizione, corpo `{promote, excluded}`); rifiuta una categoria sconosciuta, `default` in una delle due liste e un duplicato dentro `promote`, mentre un nome presente in entrambe le liste è accettato ed `excluded` vince. `label_key` e `suggest_from_moments` non sono modificabili da qui. Un contesto si assegna per album (vedi [Contesto di punteggio](#contesto-di-punteggio) nella sezione Album) o per singola foto tramite la sovrascrittura categoria qui sotto.
