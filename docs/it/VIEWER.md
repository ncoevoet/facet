...
| `GET /api/photos/paths` | `{ total, paths }` — ogni percorso corrispondente, senza ordine; usato all'occorrenza dalle azioni che hanno bisogno dei nomi file (scaricamento, copia), mai da «Seleziona tutto». Limitato a 10000: una vista che ne contiene di più viene rifiutata con un `412` che indica il conteggio e il limite anziché essere troncata — un elenco di percorsi parziale sarebbe una selezione che l'utente crede completa — quindi una vista più grande ripiega sul `GET /api/photos/count`, che restituisce solo il conteggio |
...
| Endpoint | Descrizione |
|----------|-------------|
| `POST /api/photos/batch_favorite` | Contrassegna più foto come preferite (annulla lo scarto). Il corpo è esattamente uno tra `{photo_paths}` (max 1000) o `{filters, exclude?}` — `filters` usa gli stessi parametri di query di `GET /api/photos`, come stringhe, senza limite; `exclude` è opzionale, max 1000. Non fornire né l'uno né l'altro, o fornire entrambi, restituisce un 422, e un insieme `filters` che nomina un album viene sottoposto allo stesso controllo di accesso del GET di quell'album — `404` per un album che non esiste, `403` per quello di un altro utente su un'installazione con controllo degli accessi. Risposta `{success, count}`, dove `count` è il numero di righe effettivamente scritte |
...
| `POST /api/export/sidecars` | `[Edition]` Scrivi i sidecar per percorsi espliciti o per un insieme di filtri. Corpo `{paths?, filters?, exclude?, overwrite}` — esattamente uno tra `paths` (max 10000) oppure un insieme `filters` di galleria: nominare entrambi dà un `422`, non nominarne nessuno un `400`. `exclude` (opzionale, max 1000) rimuove percorsi indicati da un insieme `filters`; `filters` viene risolto tramite la stessa normalizzazione usata da `GET /api/photos`, esattamente come `POST /api/cull/apply` sotto, e un insieme `filters` che nomina un album viene sottoposto allo stesso controllo di accesso del GET di quell'album — `404` per un album che non esiste, `403` per quello di un altro utente su un'installazione con controllo degli accessi. Un insieme `filters` che corrisponde a più di 10000 foto viene rifiutato con un `412` che indica il conteggio e il limite, contato prima che venga costruito qualunque elenco di percorsi, e mai troncato |
