...
Ogni album può portare un contesto di punteggio che decide quale categoria vince per le sue foto membro, indipendentemente dall'ordine di priorità globale — vedi [Contesti di punteggio](CONFIGURATION.md#contesti-di-punteggio). `PUT /api/albums/{id}/scoring_context` (riservato alla modalità di modifica) lo imposta e materializza lo stesso contesto su ogni foto che è membro **in questo momento**; `conflicts` nella risposta conta i membri che portavano già un contesto diverso, e `updated` conta quante ne sono state effettivamente scritte. Un album manuale risolve l'appartenenza dalle sue righe `album_photos`; un album intelligente non ne ha, quindi l'appartenenza viene invece risolta valutando il suo `smart_filter_json` salvato rispetto al database live — allo stesso modo in cui lo risolve la sua vista galleria — restituendo `updated: 0` con un `warning` quando il filtro al momento non corrisponde a nulla. In entrambi i casi la marcatura è un'istantanea, non un abbonamento continuo: una foto aggiunta *in seguito* a un album manuale eredita comunque automaticamente il contesto, ma una foto che corrisponde *più tardi* al filtro di un album intelligente **non** eredita retroattivamente il contesto — il contesto va reimpostato (`PUT`) per catturare le nuove corrispondenze. `DELETE /api/albums/{id}/scoring_context` annulla la marcatura esattamente sui membri che questo album aveva impostato, senza toccare la sovrascrittura manuale di una foto né un contesto impostato da un altro album. `GET /api/albums/{id}/suggested_context` ne propone uno a partire dal `narrative_moment` dominante rilevato per l'album (tramite l'elenco `suggest_from_moments` di ciascun contesto) con una confidenza `share` — non scrive nulla; l'assegnazione sopra descritta deve comunque essere confermata esplicitamente. Un ricalcolo (`POST /api/scan/recompute`) è necessario perché il nuovo contesto cambi effettivamente la categoria memorizzata di qualunque foto.
...
- **Contesti di punteggio** — `GET /api/config/scoring_contexts` elenca i preset configurati (`default`, `action_stage`, `party_event`, `portrait_session`, `wildlife`, `landscape`, `motorsport`) insieme all'ordine effettivo risolto di ciascuno. Un contesto promuove alcune categorie in testa ed esclude del tutto le altre senza toccare l'ordine globale, e si assegna per album (vedi [Contesto di punteggio](#contesto-di-punteggio) nella sezione Album) o per singola foto tramite la sovrascrittura categoria qui sotto.
...
| `PUT /api/albums/{id}/scoring_context` | `[Edition]` Imposta il contesto di punteggio dell'album; lo materializza sulle foto membro in questo momento (gli album intelligenti risolvono `smart_filter_json` dal vivo), restituisce `{updated, conflicts}` |
