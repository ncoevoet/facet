...
Ein Bewertungskontext wird **pro Album** über `PUT /api/albums/{id}/scoring_context` (Edition-geschützt) zugewiesen, was die `scoring_context`-Spalte des Albums schreibt und denselben Wert in `photo_scoring_overrides.scoring_context` für jedes Foto überträgt, das **in diesem Moment** Mitglied ist – der `conflicts`-Zähler in der Antwort gibt an, wie viele davon bereits einen abweichenden Kontext trugen (letzter Schreibvorgang gewinnt), und `updated` gibt an, wie viele tatsächlich geschrieben wurden. Ein manuelles Album löst die Mitgliedschaft aus seinen `album_photos`-Zeilen auf; ein intelligentes Album (`is_smart`) hat keine solchen Zeilen, daher wird die Mitgliedschaft stattdessen durch Auswertung seines gespeicherten `smart_filter_json` gegen die Live-Datenbank aufgelöst, genauso wie es die eigene Galerieansicht des Albums tut (`_resolve_album_member_paths`, das den intelligenten Zweig von `_fetch_album_photos` widerspiegelt) – ein intelligentes Album, dessen Filter derzeit nichts trifft, liefert `updated: 0` mit einer `warning` zurück. So oder so ist die Übertragung eine Momentaufnahme, kein Abonnement: Bei einem manuellen Album übernimmt ein *später* hinzugefügtes Foto den Kontext automatisch, aber ein Foto, das *später* dem Filter eines intelligenten Albums entspricht, übernimmt den Kontext **nicht** rückwirkend – nur ein erneuter Aufruf von `PUT` überträgt den Kontext auf die dann aktuellen Treffer. `DELETE /api/albums/{id}/scoring_context` löscht den `scoring_context` des Albums und macht die Übertragung auf genau den Mitgliedern rückgängig, die dieses Album markiert hat (`source = 'album:<id>'`), ohne die eigene manuelle Überschreibung eines Fotos oder einen von einem anderen Album gesetzten Kontext zu berühren. `GET /api/albums/{id}/suggested_context` schlägt einen Kontext anhand des dominanten `narrative_moment` des Albums vor, abgebildet über `suggest_from_moments`, zusammen mit dem `share`-Anteil des dominanten Moments am Album – dabei wird nichts geschrieben; die obige Zuweisung muss weiterhin explizit aufgerufen werden.
