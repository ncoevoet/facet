...
- **Bewertungskontexte** – `GET /api/config/scoring_contexts` listet die konfigurierten Voreinstellungen (`default`, `action_stage`, `party_event`, `portrait_session`, `wildlife`, `landscape`, `motorsport`) zusammen mit der aufgelösten effektiven Reihenfolge jedes einzelnen auf. Ein Kontext zieht einige Kategorien an den Anfang und schließt andere vollständig aus, ohne die globale Reihenfolge zu verändern. Das `promote`/`excluded`-Delta jeder Nicht-`default`-Voreinstellung ist selbst über denselben Tab **Bewertungskontext** bearbeitbar – den bevorzugten Kopf in die gewünschte Reihenfolge ziehen, den Ausschluss einer Kategorie umschalten, speichern – über `PUT /api/config/scoring_contexts/{name}` (Edition-geschützt, Body `{promote, excluded}`); er lehnt eine unbekannte Kategorie, `default` in einer der beiden Listen sowie ein Duplikat innerhalb von `promote` ab, während ein in beiden Listen genannter Name akzeptiert wird und `excluded` gewinnt. `label_key` und `suggest_from_moments` sind von hier aus nicht bearbeitbar. Ein Kontext wird pro Album zugewiesen (siehe [Bewertungskontext](#bewertungskontext) unter Alben) oder pro Foto über die Kategorieüberschreibung unten.
