...
Un contexto de puntuación se asigna **por álbum** mediante `PUT /api/albums/{id}/scoring_context` (requiere edición), que escribe la columna `scoring_context` del álbum y materializa el mismo valor en `photo_scoring_overrides.scoring_context` para cada foto que es miembro **en ese momento** — el `conflicts` de la respuesta indica cuántas de ellas ya llevaban un contexto distinto (gana la última escritura), y `updated` indica cuántas se escribieron realmente. Un álbum manual resuelve su pertenencia a partir de sus filas de `album_photos`; un álbum inteligente (`is_smart`) no tiene ninguna, así que su pertenencia se resuelve en su lugar evaluando su `smart_filter_json` guardado contra la base de datos en vivo, del mismo modo en que lo resuelve la propia vista de galería del álbum (`_resolve_album_member_paths`, que refleja la rama "inteligente" de `_fetch_album_photos`) — un álbum inteligente cuyo filtro no coincide actualmente con nada devuelve `updated: 0` con un `warning`. En cualquier caso, la marca es una instantánea, no una suscripción: en un álbum manual, una foto añadida *después* sí hereda el contexto automáticamente, pero una foto que coincide *más tarde* con el filtro de un álbum inteligente **no** hereda el contexto retroactivamente — solo una nueva llamada a `PUT` vuelve a marcar las coincidencias del momento. `DELETE /api/albums/{id}/scoring_context` borra el `scoring_context` del álbum y deshace la marca exactamente en los miembros que este álbum había marcado (`source = 'album:<id>'`), sin tocar la anulación manual propia de una foto ni un contexto establecido por otro álbum. `GET /api/albums/{id}/suggested_context` propone un contexto a partir del `narrative_moment` dominante del álbum, mapeado a través de `suggest_from_moments`, junto con la `share` (proporción) del momento dominante en el álbum — no escribe nada; la asignación anterior aún debe llamarse explícitamente.
