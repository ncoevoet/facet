...
`max_gap_seconds`、构图保持一致（pHash 上的 `max_hamming`），且它们的 EV
按数值排序（而非按拍摄顺序）后，构成一个由不同曝光值组成、等间距、至少
`min_frames` 帧、跨度不小于 `min_span_stops` 的阶梯。先排序意味着拍摄顺序
无关紧要：先拍基准帧的顺序（`0、-、+`，索尼、佳能、尼康都提供这种顺序）
和从暗到亮拍摄的顺序同样会被识别。正是“等间距”把包围曝光与手持连拍穿过
变化光线的漂移区分开来。
...
657 组之外再多收 381 组，而证据依然表明其中多数并不是包围曝光：62% 的跨度不足
两挡，而已确认的组里有 91% 跨度在两挡以上；额外收进的那些配对里只要出现溢出，
最常见的形态就是两帧都偏暗。
