...
Os pesos dos modelos são apenas uma parte do pico de memória — o runtime do torch, o bloco de imagem decodificada e as ativações por camada se somam a isso — então trate esses números como pisos, não como orçamentos. A linha `legacy`/`8gb` agora se apoia em testes reais em contêiner (veja abaixo); as linhas `16gb` e `24gb` continuam sendo espaços reservados provisórios, sem nenhuma medição real por trás.

| Perfil de VRAM | Pesos dos modelos (total) | Memória de contêiner recomendada |
|---|---|---|
| `legacy` / `8gb` | 15,0 GB | 12 GB (GPU) / 16 GB (CPU, provisório) |
...
O teto agora mantém a capacidade por passe em 5,0 GB não importa quão grande o limite do contêiner seja reportado, então ela para de crescer com o contêiner: o perfil `8gb` em CPU sempre planeja os mesmos 5 passes independentemente do limite — `Pass 1: qrealign [~5.0GB RAM]`, `Pass 2: clip + topiq_iaa [~5.0GB RAM]`, `Pass 3: topiq_nr_face + liqe [~4.0GB RAM]`, `Pass 4: saliency + samp_net [~4.0GB RAM]`, `Pass 5: insightface [~2.0GB RAM]`. Essa forma fixa ainda assim não cabe em um contêiner de 8 GB: testes reais ainda resultam em `OOMKilled` (código de saída 137), no passe 4, com um pico de 7,67 GB dos 8 GB do orçamento. Um contêiner de 16 GB vai além do ponto em que os 8 GB e os 12 GB falhavam — essa execução ainda estava em andamento na última verificação, então o que segue é um piso, não um pico final. Seu cgroup já mostrava pelo menos 12,55 GB de memória anônima, o número que o OOM killer do kernel realmente cobra: nem o MemUsage do `docker stats` nem o `memory.current` do cgroup, que contam ambos o cache de páginas recuperável, de modo que o primeiro subestima o risco real e o segundo fica preso perto do limite do contêiner independentemente de quanta margem realmente resta. Esses mesmos 12,55 GB também explicam por que a execução de 12 GB acima foi morta, e isso bate com o pico de 9,23 GB relatado pelo autor do issue em GPU — o mesmo catálogo de modelos, menos o que fica na VRAM em vez de na RAM do contêiner. O número de 16 GB na tabela acima reflete esse piso, não um teto final confirmado — por isso continua rotulado como provisório. Um usuário de GPU que dimensionasse com base nos números de CPU aqui superdimensionaria; um usuário de CPU que dimensionasse com base no número de GPU subdimensionaria — use o que corresponder a como seu contêiner realmente roda.
