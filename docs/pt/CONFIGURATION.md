...
Um contexto de pontuação é atribuído **por álbum** via `PUT /api/albums/{id}/scoring_context` (protegido por edition), que grava a coluna `scoring_context` do álbum e materializa o mesmo valor em `photo_scoring_overrides.scoring_context` para cada foto que é membro **naquele momento** — a contagem `conflicts` na resposta indica quantas delas já carregavam um contexto diferente (a última gravação prevalece), e `updated` indica quantas foram realmente gravadas. Um álbum manual resolve sua composição a partir das linhas de `album_photos`; um álbum inteligente (`is_smart`) não tem nenhuma, então sua composição é resolvida em vez disso avaliando seu `smart_filter_json` salvo contra o banco de dados ao vivo, da mesma forma que a própria visão de galeria do álbum a resolve (`_resolve_album_member_paths`, que espelha o ramo "inteligente" de `_fetch_album_photos`) — um álbum inteligente cujo filtro atualmente não corresponde a nada retorna `updated: 0` com um `warning`. De qualquer forma, a marcação é um instantâneo, não uma assinatura: em um álbum manual, uma foto adicionada *depois* herda o contexto automaticamente, mas uma foto que passa a corresponder *depois* ao filtro de um álbum inteligente **não** herda o contexto retroativamente — apenas uma nova chamada a `PUT` remarca as correspondências atuais. `DELETE /api/albums/{id}/scoring_context` apaga o `scoring_context` do álbum e desfaz a marcação exatamente nos membros que este álbum havia marcado (`source = 'album:<id>'`), sem tocar na substituição manual própria de uma foto nem em um contexto definido por outro álbum. `GET /api/albums/{id}/suggested_context` propõe um contexto a partir do `narrative_moment` dominante do álbum, mapeado através de `suggest_from_moments`, junto com o `share` (participação) do momento dominante no álbum — ele não grava nada; a atribuição acima ainda precisa ser chamada explicitamente.
