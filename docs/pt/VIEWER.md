...
| `POST /api/cull/apply` | `[Edition]` Copia as mantidas ou move/descarta para a lixeira as rejeitadas para uma pasta (veja [Selecionar para pasta](#selecionar-para-pasta)). Corpo `{paths?, filters?, action, target_dir?, include_companions, include_sequence_siblings, dry_run}` — ou `paths`, ou um conjunto `filters` da galeria; `action` é `copy_keeps \| trash_rejects \| move_rejects`; `include_companions` (padrão `false`) inclui o RAW acompanhante de mesmo nome-base e o `.xmp` de cada arquivo; `include_sequence_siblings` (padrão `false`) inclui todo outro fotograma que compartilhe o `(sequence_kind, sequence_group_id)` de uma foto correspondida **cujo próprio estado de rejeição corresponda à ação**, de modo que um fotograma mantido nunca é destruído porque um irmão foi rejeitado; `dry_run` tem padrão `true`. A resposta acrescenta `matched` (caminhos que se qualificaram para a ação) e `sequence_siblings` (contagem de fotogramas irmãos, informada mesmo com a opção desativada) além de `skipped` / `excluded_by_state` / `not_visible` |
