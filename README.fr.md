...
1. **Analyse** — Pointez Facet vers un dossier de photos. Chaque image est analysée pour sa qualité, sa composition et ses visages. Prend en charge le JPG, le HEIF/HEIC/HIF (les images `.HIF` Canon HDR PQ sont décodées avec la fonction de transfert SMPTE ST 2084 PQ et tonemappées automatiquement en sRGB SDR), 10 formats RAW (CR2, CR3, NEF, ARW, RAF, RW2, DNG, ORF, SRW, PEF), ainsi que les images PNG, GIF, WebP, BMP, TIFF et AVIF (AVIF nécessite un Pillow compilé avec la prise en charge AVIF, native à partir de 11.3). Un RAW que LibRaw ne peut pas décoder — comme un DNG « Fusionner en HDR/Panorama » de Lightroom (`-HDR.dng`, `-Pano.dng`) ou un appareil plus récent que la version de LibRaw installée — bascule sur son plus grand aperçu intégré via `exiftool` (nécessite un aperçu d'au moins 1024 px sur le grand côté ; les scores viennent alors de cet aperçu, pas d'un décodage RAW complet).
